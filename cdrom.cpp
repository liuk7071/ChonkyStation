...
void cdrom::INT1(void* dataptr) {
	...
	if (!cdromptr->cancel_int1) {
		...
		bool form2 = (cdromptr->cd.SectorBuffer[0x12] & 32);
		...
		if (!cdromptr->xa_adpcm || !(audio_sector && realtime)) {
			...
			cdromptr->response_fifo[0] = cdromptr->get_stat();
			cdromptr->response_length = 1;

			cdromptr->status |= 0b00100000;
			cdromptr->cd.bytes_read = 0;
		}
		cdromptr->Scheduler.push(&INT1, cdromptr->Scheduler.time + ((33868800 / 75) / (cdromptr->DoubleSpeed ? 2 : 1)), cdromptr);
	}
	else {
		cdromptr->cancel_int1 = false;
	}
}
...
void cdrom::GetTD() {
	...
	// Some hardcoding...
	if (fifo[0] == 0) {
		printf("[CDROM] GetTD: Got track 00h, calculating CD lba (assuming only 1 track exists, no cue parsing yet)\n");
		printf("[CDROM] GetTD: File size: %d\n", cd.file_size);
		lba += cd.file_size / SECTOR_SIZE;
		printf("[CDROM] GetTD: lba = %d\n", lba);
	}
	response_fifo[1] = (lba / 75) / 60;
	response_fifo[2] = (lba / 75) % 60;
	response_length = 3;
	status |= 0b00100000;
	Scheduler.push(&INT3, Scheduler.time + 50401, this);
}
...
void cdrom::GetID() { // Disk info
	...
	if (cd.IsCDInserted) {
		...
		Scheduler.push(&INT2, Scheduler.time + 50401 + 18944, this);
	}
	else {
		response_fifo[0] = get_stat();
		response_length = 1;
...
void cdrom::SetLoc() { // Sets the seek target
	...
	Scheduler.push(&INT3, Scheduler.time + 18491, this);
	status |= 0b00100000;
}
void cdrom::Play() {
	status |= 0b00001000;
...
void cdrom::ReadN() {	// Read with retry
	...
	Scheduler.push(&INT3, Scheduler.time + 50401, this);
	Scheduler.push(&INT1, Scheduler.time + INT1_DELAY, this);
...
void cdrom::Pause() {
	...
	Scheduler.push(&INT3, Scheduler.time + 50401, this);
	Scheduler.push(&INT2, Scheduler.time + 50401 + 1097107, this);
